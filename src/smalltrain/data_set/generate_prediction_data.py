...
def get_report_dir_path(save_root_dir, train_id):
    ...
    GGPrint.print('{} {} {}'.format(save_root_dir, train_id, report_dir_path))
...
def generate_prediction_data(report_dir_path, target_datetime_str, target_datetime_col_name, target_value_col_name, second_key_col_name, prediction_dt_col_name, prediction_file_date_format='%Y-%m-%d', denormalize_value_ratio=None):
    ...
    for _offset in list(range(MIN_OFFSET, MAX_OFFSET+1)):
        ...
        GGPrint.print('check prediction file for a path:'.format(prediction_file_path))
    ...
    if offset_to_use is None:
        ...
        GGPrint.print('Use offset:{} to get prediction file'.format(offset_to_use))
    # GGPrint.print('prediction_file_path_list:{}'.format(prediction_file_path_list))
    ...
    for prediction_file_path in prediction_file_path_list:
        ...
        if not os.path.isfile(prediction_file_path):
            GGPrint.print('Skip to read non-exist file. prediction_file_path:{}'.format(prediction_file_path))
        ...
        if len(target_datetime_series) == 1:
            ...
            GGPrint.print('target_datetime_str:{}, prediction_value:{}, second_key_value:{}'.format(target_datetime_str, prediction_value, second_key_value))
...
if __name__ == '__main__':
    ...
    GGPrint.print(df_selected_with_second_key)
    ...
    if (args.s3_bucket_name is not None):
        GGPrint.print('Upload prediction_data_file_name:{} to s3_bucket_name:{}'.format(prediction_data_file_name, args.s3_bucket_name))
