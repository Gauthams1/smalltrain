...
class IMGDataSet(GGDataSet):
    ...
    def __init__(self, debug_mode=False,
        ...
        if hparams and 'max_data_per_img' in hparams.keys():
            GGPrint.print('{}Use max_data_per_img in hparams:{}'.format(PREFIX, hparams['max_data_per_img']))
            self.max_data_per_img = hparams['max_data_per_img']
        else:
            GGPrint.print('{}TODO Use max_data_per_img with default value:{}'.format(PREFIX, self.max_data_per_img))
    ...
    def generate_input_output_data(self):
        ...
        if 'label' in self.output_data_names:
            ...
            GGPrint.print('max_label: {}, len of target_label_in_use: {}'.format(max_label, len(self.target_label_in_use)))
        else:
            # set output_classes if not given with hyper parameter
    ...
    def generate_cache_data(self):
        ...
        for _data_set_index, _index in enumerate(self.df_data_set_def.index):
            ...
            def create_img_data(data_set_file, label_in_use,
                ...
                if self.use_cache and not witout_check_cache:
                    ...
                    try:
                        ...
                        # GGPrint.print('len(img_data):{}'.format(len(img_data)))
                        # GGPrint.print('Checking img_data with key:{}, _check:{}'.format(key, _check))
                        assert _check
                ...
                try:
                    ...
                    GGPrint.print('error: {} with read img file:{}'.format(e, data_set_file))
                    return None
                GGPrint.print('DONE img_data read')

                # debug
                # if self.debug_mode:
                #     GGPrint.print('img_data first:', img_data[0][0][0])
                #     GGPrint.print('img_data last:', img_data[-1][-1][-1])
            ...
            GGPrint.print('========== start with data_index:{} =========='.format(data_index))
            ...
            GGPrint.print('work_img_data.shape:{}'.format(work_img_data.shape))
            ...
            try:
                ...
                GGPrint.print('warning. an Exception occured in setting target_ts_list with self.max_data_per_img:{}. target_ts_list will be set as default. Exception:{}'.format(self.max_data_per_img, e))
                target_ts_list = np.arange(target_ts, target_ts_end + 1)

            if self.debug_mode:
                GGPrint.print('target_ts_list:'.format(target_ts_list))
            ...
            for target_ts in target_ts_list:
                ...
                def create_each_input_data(work_img_data, target_ts, input_img_width,
                    ...
                    # GGPrint.print('target_ts:{}'.format(target_ts))
                    # GGPrint.print('input_img_width:{}'.format(input_img_width))
                    ...
                    try:
                        ...
                        if self.skip_invalid_data is None or (self.skip_invalid_data is not None and self.is_valid_data(output_data.get(data_index))):
                            if self.is_test_data(test_flag, target_ts - 1):
                                if only_debug_at_first_in_roop: GGPrint.print('add to test data. target_ts:{}'.format(target_ts))
                                self.test_index_list.append(data_index)
                            else:
                                if only_debug_at_first_in_roop: GGPrint.print('add to train data. target_ts:{}'.format(target_ts))
                        ...
                        _do_nothing = 0
                        if only_debug_at_first_in_roop: GGPrint.print('do not add to any data. train data. target_ts:{}'.format(target_ts))
