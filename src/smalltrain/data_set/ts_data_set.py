...
class TSDataSet(GGDataSet):
    ...
    def __init__(self, debug_mode=False,
        ...
        if hparams and 'ts_start' in hparams.keys():
            GGPrint.print('{}Use ts_start in hparams:{}'.format(PREFIX, hparams['ts_start']))
            self.ts_start = hparams['ts_start']
        else:
            GGPrint.print('{}TODO Use ts_start with default value:{}'.format(PREFIX, self.ts_start))

        # self.ts_end = 50000 # memory error
        self.ts_end = 10000
        if hparams and 'ts_end' in hparams.keys():
            GGPrint.print('{}Use ts_end in hparams:{}'.format(PREFIX, hparams['ts_end']))
            self.ts_end = hparams['ts_end']
        else:
            GGPrint.print('{}TODO Use ts_end with default value:{}'.format(PREFIX, self.ts_end))

        self.test_ts_index_from = None
        if hparams and 'test_ts_index_from' in hparams.keys():
            GGPrint.print('{}Use test_ts_index_from in hparams:{}'.format(PREFIX, hparams['test_ts_index_from']))
        ...
        if self.test_ts_index_from is None:
            ...
            GGPrint.print('{}TODO Use test_ts_index_from with default value:{}'.format(PREFIX, self.test_ts_index_from))

        self.test_ts_index_to = None
        if hparams and 'test_ts_index_to' in hparams.keys():
            GGPrint.print('{}Use test_ts_index_to in hparams:{}'.format(PREFIX, hparams['test_ts_index_to']))
            self.test_ts_index_to = hparams['test_ts_index_to']
        if self.test_ts_index_to is None:
            self.test_ts_index_to = self.ts_end
            GGPrint.print('{}Use test_ts_index_to with default value:{}'.format(PREFIX, self.test_ts_index_to))
        ...
        if hparams and 'max_data_per_ts' in hparams.keys():
            GGPrint.print('{}Use max_data_per_ts in hparams:{}'.format(PREFIX, hparams['max_data_per_ts']))
            self.max_data_per_ts = hparams['max_data_per_ts']
        else:
            GGPrint.print('{}TODO Use max_data_per_ts with default value:{}'.format(PREFIX, self.max_data_per_ts))

        self.dt_col_name = DT_COL_NAME
        if hparams and 'dt_col_name' in hparams.keys():
            GGPrint.print('{}Use dt_col_name in hparams:{}'.format(PREFIX, hparams['dt_col_name']))
            self.dt_col_name = hparams['dt_col_name']
        else:
            GGPrint.print('{}No dt_col_name is set'.format(PREFIX))

        self.col_size = len(self.input_data_names)

        self.dt_col_format = None
        if hparams and 'dt_col_format' in hparams.keys():
            GGPrint.print('{}Use dt_col_format in hparams:{}'.format(PREFIX, hparams['dt_col_format']))
            self.dt_col_format = hparams['dt_col_format']
        else:
            GGPrint.print('{}Error no dt_col_format'.format(PREFIX))
            exit(1)

        self.dt_unit = None
        if hparams and 'dt_unit' in hparams.keys():
            GGPrint.print('{}Use dt_unit in hparams:{}'.format(PREFIX, hparams['dt_unit']))
            self.dt_unit = hparams['dt_unit']
        else:
            GGPrint.print('{}Error no dt_unit'.format(PREFIX))
            exit(1)

        self.dt_unit_per_sec = TSDataSet.get_seconds_with_dt_unit(dt_unit=self.dt_unit, time_step=1)
        GGPrint.print('self.dt_unit_per_sec:{}'.format(self.dt_unit_per_sec))

        # about add_dt_col_name_list
        self.add_dt_col_name_list = None
        if hparams and 'add_dt_col_name_list' in hparams.keys():
            GGPrint.print('{}Use add_dt_col_name_list in hparams:{}'.format(PREFIX, hparams['add_dt_col_name_list']))
            ...
            if self.add_dt_col_name_list is not None:
                ...
                # GGPrint.print('len_of_add_dt_col_name_list:{}'.format(len_of_add_dt_col_name_list))
                ...
                for add_dt_col_name in self.add_dt_col_name_list:
                    try:
                        ...
                        GGPrint.print('{}add_dt_col_name_list col_name:{}, func:{}, args:{}'.format(PREFIX, col_name, func, args))
                    except KeyError:
                        GGPrint.print('{}Error on invalid parameter:{}'.format(PREFIX, add_dt_col_name))
                        exit(1)
        else:
            GGPrint.print('{}No dt_col_name is set'.format(PREFIX))
        ...
        if hparams and 'input_output_ts_offset' in hparams.keys():
            GGPrint.print('{}Use input_output_ts_offset in hparams:{}'.format(PREFIX, hparams['input_output_ts_offset']))
            self.input_output_ts_offset = hparams['input_output_ts_offset']
        else:
            GGPrint.print('{}Use input_output_ts_offset with default value:{}'.format(PREFIX, self.input_output_ts_offset))
        ...
        if hparams and 'input_output_ts_offset_range' in hparams.keys():
            GGPrint.print('{}Use input_output_ts_offset_range in hparams:{}'.format(PREFIX, hparams['input_output_ts_offset_range']))
            self.input_output_ts_offset_range = hparams['input_output_ts_offset_range']
        else:
            GGPrint.print('{}No input_output_ts_offset_range'.format(PREFIX))

        self.input_output_ts_offset_list = [self.input_output_ts_offset]
        if hparams and 'input_output_ts_offset_list' in hparams.keys():
            GGPrint.print('{}Use input_output_ts_offset_list in hparams:{}'.format(PREFIX, hparams['input_output_ts_offset_list']))
            self.input_output_ts_offset_list = hparams['input_output_ts_offset_list']
        if self.input_output_ts_offset_list is None:
            GGPrint.print('{}TODO reset input_output_ts_offset_list because input_output_ts_offset_list set None.'.format(PREFIX))
            if self.input_output_ts_offset_range is not None:
                ...
                self.input_output_ts_offset_list = [self.input_output_ts_offset]
        GGPrint.print('{}input_output_ts_offset_list is set :{}'.format(PREFIX, self.input_output_ts_offset_list))
        ...
        GGPrint.print('self.add_offset_column is:{} because of len(self.input_output_ts_offset_list):{}'.format(self.add_offset_column, len(self.input_output_ts_offset_list)))
        if self.add_offset_column is not None:
            self.add_offset_column = True
            self.col_size += 1
            self.offset_column_index = (self.col_size - 1)
        GGPrint.print('{}add_offset_column:{}'.format(PREFIX, self.add_offset_column))
    ...
    def generate_input_output_data(self):
        ...
        # GGPrint.print('train_ts_width:{}'.format(self.train_ts_width))
        self.data_size_reserved = self.train_ts_width * data_set_cnt * len(self.input_output_ts_offset_list)
        GGPrint.print('self.data_size_reserved:{}'.format(self.data_size_reserved))
    ...
    def generate_cache_data(self):
        ...
        if not self.prepare_data_mode:
            GGPrint.print('self.col_size to init input_data:{}'.format(self.col_size))
            input_data = TSData(name='input_data_{}'.format(self.cache_data_set_id), db_host=self.cache_db_host, refresh=True)
        ...
        for _data_set_index, _index in enumerate(self.df_data_set_def.index):
            ...
            if 'label' in self.output_data_names:
                ...
                if self.model_type == 'CLASSIFICATION':
                    ...
                    GGPrint.print('label_v:{}'.format(label_v))

            else:
                GGPrint.print('do regression or classification to output_data_names:'.format(self.output_data_names))
            ...
            def calc_size_to_complement_before(input_ts_width, target_ts_start, multi_resolution_channels,
                ...
                GGPrint.print('size_to_complement_before:{}'.format(size_to_complement_before))
            ...
            def create_ts_data(data_set_file, dt_col_name, dt_unit, dt_unit_per_sec, label_in_use,
                ...
                if self.use_cache and not witout_check_cache:
                    ...
                    try:
                        ...
                        # GGPrint.print('len(ts_data):{}'.format(len(ts_data)))
                        # GGPrint.print('Checking ts_data with key:{}, _check:{}'.format(key, _check))
                        assert _check
                        GGPrint.print('Get cache with key:{}'.format(key))
                        return ts_data
                    except Exception as e:
                        # GGPrint.print('No cache with key:{} with Exception:{}'.format(key, e))
                        None
                ...
                try:
                    ...
                    GGPrint.print('error: {} with read csv:{}'.format(e, data_set_file))
                    return None
                GGPrint.print('DONE ts_data read')
                ...
                if dt_col_name:
                    ...
                    if isinstance(ts_data[dt_col_name][0], str):
                        GGPrint.print('TODO ts_data dt col parse_datetime')
                        if dt_unit.find('relative') >= 0:
                            ...
                            # GGPrint.print(ts_data[dt_col_name])
                        else:
                            ts_data[dt_col_name] = [parse_datetime(str_dt) for str_dt in ts_data[dt_col_name]]
                        GGPrint.print('DONE ts_data dt col parse_datetime')
                    ts_data.sort_values(by=[dt_col_name])
                    GGPrint.print('DONE ts_data.sort_values')
                ...
                GGPrint.print('DONE ts_data.reset_index')

                # debug
                if self.debug_mode:
                    GGPrint.print('ts_data first:', ts_data.iloc[0])
                    GGPrint.print('ts_data last:', ts_data.iloc[-1])
                ...
                if self.debug_mode:
                    GGPrint.print('self.complement_input_data:{}'.format(self.complement_input_data))
                if self.complement_input_data is not None:
                    ...
                    ts_data = ts_data.reset_index()
                    GGPrint.print('----- after self.complement_input_data:{} -----'.format(self.complement_input_data))
                    GGPrint.print('head rows:{}'.format(ts_data.iloc[:21]))
                    GGPrint.print('----------')
                ...
                if not has_to_complement_before:
                    ...
                    GGPrint.print('first_row:{}'.format(first_row))

                    l = len(ts_data)
                    GGPrint.print('##### before complementation #####')
                    GGPrint.print('head rows:{}'.format(ts_data.iloc[:5]))
                    GGPrint.print('dt_col_name:{}'.format(dt_col_name))
                    ...
                    for i in range(1, size_to_complement_before):
                        ...
                        if i % 1000 == 0:
                            GGPrint.print('DONE complementation index:{}/{} ({:.1f}%)'.format(i, size_to_complement_before,
                                                                                      100 * i / size_to_complement_before))

                    ts_data = pd.concat([df_complement, ts_data])
                    ts_data = ts_data.sort_index()
                    GGPrint.print('##### after complementation #####')
                ...
                if self.use_cache:
                    ...
                    GGPrint.print('Set cache with key:{}'.format(key))
            ...
            if self.prepare_data_mode:
                if data_index % 1000 == 0:
                    GGPrint.print('prepare_data_mode data_index:{}/{}'.format(data_index, self.data_size_reserved))
                data_index += 1
                continue
            ...
            for _input_output_ts_offset in self.input_output_ts_offset_list:
                ...
                GGPrint.print('========== _input_output_ts_offset:{} start with data_index:{} =========='.format(_input_output_ts_offset, data_index))
                ...
                def create_work_ts_data(ts_data, dt_col_name, add_dt_col_name_list, dt_unit, each_input_output_ts_offset):
                    ...
                    if self.use_cache and not witout_check_cache:
                        ...
                        try:
                            # check cache
                            # GGPrint.print('len(work_ts_data):{}'.format(len(work_ts_data)))
                            for i in range(min(2, len(work_ts_data))):
                                _check = work_ts_data.iloc[i][dt_col_name] == ts_data.iloc[i][dt_col_name]
                                # GGPrint.print('Checking cache with key:{}, _check:{}'.format(key, _check))
                                assert _check
                    ...
                    if dt_col_name is not None and add_dt_col_name_list is not None:
                        GGPrint.print('##### before add datetime columns #####')
                        GGPrint.print('head rows:{}'.format(work_ts_data.iloc[:8]))
                        ...
                        for add_dt_col_name in add_dt_col_name_list:
                            ...
                            GGPrint.print('add_dt_col_name_list col_name:{}, func:{}, args:{}'.format(col_name, func, args))
                        ...
                        GGPrint.print('##### after add datetime columns #####')
                        GGPrint.print('head rows:{}'.format(work_ts_data.iloc[:21]))
                ...
                try:
                    ...
                    GGPrint.print('warning. an Exception occured in setting target_ts_list with self.max_data_per_ts:{}. target_ts_list will be set as default. Exception:{}'.format(self.max_data_per_ts, e))
                    target_ts_list = np.arange(target_ts, target_ts_end + 1)
                for target_ts in target_ts_list:

                    if data_index % 1000 == 0:
                        GGPrint.print('data_index:{}/{} ({:.1f}%), target_ts:{}'.format(data_index, self.data_size_reserved, 100 * data_index / self.data_size_reserved, target_ts))
                    ...
                    def create_each_input_data(work_ts_data, target_ts, input_ts_width, each_input_output_ts_offset,
                        ...
                        if self.use_cache and not witout_check_cache:
                            ...
                            try:
                                # check cache
                                GGPrint.print('len(_input_data):{}'.format(len(_input_data)))
                                _check = (len(_input_data) == input_ts_width)
                                # GGPrint.print('Checking cache len(_input_data) with key:{}, _check:{}'.format(key, _check))
                                assert _check
                                GGPrint.print('Get cache with key:{}'.format(key))
                                return _input_data
                            except Exception as e:
                        ...
                        try:
                            ...
                            GGPrint.print('AssertionError len(input_filterd):{}'.format(len(input_filterd)))
                            exit(1)
                        ...
                        if multi_resolution_channels > 0:
                            for extend_level in range(1, multi_resolution_channels + 1):
                                # if only_debug_at_first_in_roop: GGPrint.print('extend_level:{}'.format(extend_level))
                                ...
                                try:
                                    ...
                                    GGPrint.print('AssertionError len(_input_ts_index):{}'.format(len(_input_ts_index)))
                                    exit(1)
                        ...
                        if add_offset_column:
                            # GGPrint.print('each_input_output_ts_offset:{}'.format(each_input_output_ts_offset))
                    ...
                    if only_debug_at_first_in_roop:
                        GGPrint.print('_input_data.shape:{}'.format(_input_data.shape))
                        GGPrint.print('_input_data:{}'.format(_input_data[:5]))
                    ...
                    if label_v is not None: GGPrint.print('label_v shape:{}'.format(label_v.shape))
                    # GGPrint.print('output_data shape:{}'.format(output_data.shape()))
                    ...
                    if self.annotation_col_names is not None:
                        ...
                        def create_each_annotation_data(work_ts_data, each_output_data, target_ts, annotation_col_names, prediction_mode):
                            ...
                            # if only_debug_at_first_in_roop:
                            #     GGPrint.print('annotation_data_filterd_from_input ==========')
                            #     GGPrint.print(annotation_data_filterd_from_input)
                            #     GGPrint.print('annotation_data of data_index ==========')
                            #     GGPrint.print(annotation_data.get(data_index))
                    ...
                    if self.prediction_mode:
                        ...
                        try:
                            # skip invalid data
                            if only_debug_at_first_in_roop: GGPrint.print('self.skip_invalid_data:{}, self.valid_data_range:{}'.format(self.skip_invalid_data, self.valid_data_range))
                            if self.skip_invalid_data is None or (self.skip_invalid_data is not None and self.is_valid_data(output_data.get(data_index))):
                                if self.is_test_data(test_flag, target_ts - size_to_complement_before + _input_output_ts_offset - 1):
                                    if only_debug_at_first_in_roop: GGPrint.print('add to test data. target_ts:{}, size_to_complement_before:{}, _input_output_ts_offset:{}'.format(target_ts, size_to_complement_before, _input_output_ts_offset))
                                    self.test_index_list.append(data_index)
                                else:
                                    if only_debug_at_first_in_roop: GGPrint.print('add to train data. target_ts:{}, _input_output_ts_offset:{}'.format(target_ts, _input_output_ts_offset))
                                    self.train_index_list.append(data_index)
                            else:
                                if only_debug_at_first_in_roop: GGPrint.print('skip output_data.get(data_index):{}'.format(output_data.get(data_index)))

                        except Exception:
                            _do_nothing = 0
                            if only_debug_at_first_in_roop: GGPrint.print('do not add to any data. train data. target_ts:{}, _input_output_ts_offset:{}'.format(target_ts,
        ...
        if self.multiprocessing:
            while len(self.thread_dict) > 0:
                GGPrint.print('waiting for thread finish with len:{}'.format(len(self.thread_dict)))
                time.sleep(1)

        # if self.annotation_col_names is not None:
        #     annotation_data = annotation_data[:data_index]

        GGPrint.print('DONE cut data with data_index')

        # TODO GGPrint.print('select_train_data_set_with_key_time / generate_ts_time:{}'.format(select_train_data_set_with_key_time / generate_ts_time))

        if self.prepare_data_mode:
            GGPrint.print('DONE prepare_data_mode')
            exit()
        ...
        GGPrint.print('DONE generate_input_output_data with input_data.get_size():{}'.format(input_data.get_size()))
