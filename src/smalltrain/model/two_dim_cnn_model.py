...
class TwoDimCNNModel(OneDimCNNModel):
    ...
    def set_hparams_on_res_net(self, hparams):
        ...
        if hparams and 'has_res_net' in hparams.keys():
            GGPrint.print('Use has_res_net in hparams:{}'.format(hparams['has_res_net']))
            self.has_res_net = hparams['has_res_net']
        else:
            GGPrint.print('Use has_res_net with default value:{}'.format(self.has_res_net))
    ...
    def set_num_cnn_layers_in_res_block(self, hparams):
        ...
        try:
            if hparams and 'num_cnn_layers_in_res_block' in hparams.keys():
                GGPrint.print('Use num_cnn_layers_in_res_block in hparams:{}'.format(hparams['num_cnn_layers_in_res_block']))
            ...
            self.num_cnn_layers_in_res_block = DEFAULT_NUM_CNN_LAYERS_IN_RES_BLOCK
            GGPrint.print('Use num_cnn_layers_in_res_block with default value:{} because of error:{}'.format(
    ...
    def set_flip_randomly_left_right(self, hparams):
        ...
        if hparams and 'flip_randomly_left_right' in hparams.keys():
            GGPrint.print('Use flip_randomly_left_right in hparams:{}'.format(hparams['flip_randomly_left_right']))
    ...
    def set_crop_randomly_and_size(self, hparams):
        ...
        if hparams and 'crop_randomly' in hparams.keys():
            GGPrint.print('Use crop_randomly in hparams:{}'.format(hparams['crop_randomly']))
        ...
        if self.crop_randomly:
            try:
                if hparams and 'size_random_crop_from' in hparams.keys():
                    GGPrint.print('Use size_random_crop_from in hparams:{}'.format(hparams['size_random_crop_from']))
                ...
                GGPrint.print('Use size_random_crop_from with default value:{} because of error:{}'.format(
    ...
    def set_rotate(self, hparams):
        ...
        if hparams and 'rounding_angle' in hparams.keys():
            GGPrint.print('Use rounding_angle in hparams:{}'.format(hparams['rounding_angle']))
        ...
        if hparams and 'angle_rotate_randomly' in hparams.keys():
            GGPrint.print('Use angle_rotate_randomly in hparams:{}'.format(hparams['angle_rotate_randomly']))
    ...
    def set_resize_to_crop_with(self, hparams):
        ...
        if hparams and 'resize_to_crop_with' in hparams.keys():
            GGPrint.print('Use resize_to_crop_with in hparams:{}'.format(hparams['resize_to_crop_with']))
    ...
    def set_params(self, log_dir_path, model_id=None, train_data=None, debug_mode=True, prediction_mode=False, hparams=None):
        ...
        if hparams and 'monochrome_mode' in hparams.keys():
            GGPrint.print('{}Use monochrome_mode in hparams:{}'.format(PREFIX, hparams['monochrome_mode']))
            self.monochrome_mode = hparams['monochrome_mode']
        else:
            GGPrint.print('{}TODO Use monochrome_mode with default value'.format(PREFIX))
        ...
        if hparams and 'input_img_width' in hparams.keys():
            GGPrint.print('{}Use input_img_width in hparams:{}'.format(PREFIX, hparams['input_img_width']))
            self.input_img_width = hparams['input_img_width']
        else:
            GGPrint.print('{}TODO Use input_img_width with default value'.format(PREFIX))
        ...
        if hparams and 'cnn_weight_stddev_list' in hparams.keys():
            GGPrint.print('{}Use cnn_weight_stddev_list in hparams:{}'.format(PREFIX, hparams['cnn_weight_stddev_list']))
        ...
        GGPrint.print('{}cnn_weight_stddev_list is set: {}'.format(PREFIX, self.cnn_weight_stddev_list))
        ...
        if hparams and 'cnn_bias_value_list' in hparams.keys():
            GGPrint.print('{}Use cnn_bias_value_list in hparams:{}'.format(PREFIX, hparams['cnn_bias_value_list']))
        ...
        GGPrint.print('{}cnn_bias_value_list is set: {}'.format(PREFIX, self.cnn_bias_value_list))

        self.num_add_fc_layers = 0
        if hparams and 'num_add_fc_layers' in hparams.keys():
            GGPrint.print('{}Use num_add_fc_layers in hparams:{}'.format(PREFIX, hparams['num_add_fc_layers']))
            self.num_add_fc_layers = hparams['num_add_fc_layers']
        else:
            GGPrint.print('{}Use num_add_fc_layers with default value:{}'.format(PREFIX, self.num_add_fc_layers))

        self.fc_node_size_list = None

        if hparams and 'fc_node_size_list' in hparams.keys():
            GGPrint.print('{}Use fc_node_size_list in hparams:{}'.format(PREFIX, hparams['fc_node_size_list']))
        ...
        GGPrint.print('{}fc_node_size_list is set: {}'.format(PREFIX, self.fc_node_size_list))

        self.fc_weight_stddev_list = None
        if hparams and 'fc_weight_stddev_list' in hparams.keys():
            GGPrint.print('{}Use fc_weight_stddev_list in hparams:{}'.format(PREFIX, hparams['fc_weight_stddev_list']))
        ...
        GGPrint.print('{}fc_weight_stddev_list is set: {}'.format(PREFIX, self.fc_weight_stddev_list))

        self.fc_bias_value_list = None
        if hparams and 'fc_bias_value_list' in hparams.keys():
            GGPrint.print('{}Use fc_bias_value_list in hparams:{}'.format(PREFIX, hparams['fc_bias_value_list']))
        ...
        GGPrint.print('{}fc_bias_value_list is set: {}'.format(PREFIX, self.fc_bias_value_list))
        ...
        if hparams and 'has_minmax_norm' in hparams.keys():
            GGPrint.print('{}Use has_minmax_norm in hparams:{}'.format(PREFIX, hparams['has_minmax_norm']))
            self.has_minmax_norm = hparams['has_minmax_norm']
        else:
            GGPrint.print('{}Use has_minmax_norm with default value:{}'.format(PREFIX, self.has_minmax_norm))

        if self.has_minmax_norm:
            self.input_min = None
            try:
                if hparams and 'input_min' in hparams.keys():
                    GGPrint.print('{}Use input_min in hparams:{}'.format(PREFIX, hparams['input_min']))
                    self.input_min = float(hparams['input_min'])
            except (TypeError, ValueError) as e:
                self.input_min = None
                GGPrint.print('{}Use input_min from input data'.format(PREFIX))
            self.input_max = None
            try:
                if hparams and 'input_max' in hparams.keys():
                    GGPrint.print('{}Use input_max in hparams:{}'.format(PREFIX, hparams['input_max']))
                    self.input_max = float(hparams['input_max'])
            except (TypeError, ValueError) as e:
                self.input_max = None
                GGPrint.print('{}Use input_max from input data'.format(PREFIX))
        ...
        if hparams and 'test_report_frequency' in hparams.keys():
            GGPrint.print('{}Use test_report_frequency in hparams:{}'.format(PREFIX, hparams['test_report_frequency']))
            self.test_report_frequency = hparams['test_report_frequency']
        try:
            self.test_report_frequency = int(self.test_report_frequency)
        except (ValueError, TypeError) as e:
            self.test_report_frequency = 100
            GGPrint.print('{}Use test_report_frequency with default value:{} because of error:{}'.format(PREFIX, self.test_report_frequency, e))

        self.train_report_frequency = 100
        if hparams and 'train_report_frequency' in hparams.keys():
            GGPrint.print('{}Use train_report_frequency in hparams:{}'.format(PREFIX, hparams['train_report_frequency']))
            self.train_report_frequency = hparams['train_report_frequency']
        try:
            self.train_report_frequency = int(self.train_report_frequency)
        except (ValueError, TypeError) as e:
            self.train_report_frequency = 100
            GGPrint.print('{}Use train_report_frequency with default value:{} because of error:{}'.format(PREFIX, self.train_report_frequency, e))

        self.save_model_frequency = 100
        if hparams and 'save_model_frequency' in hparams.keys():
            GGPrint.print('{}Use save_model_frequency in hparams:{}'.format(PREFIX, hparams['save_model_frequency']))
            self.save_model_frequency = hparams['save_model_frequency']
        try:
            self.save_model_frequency = int(self.save_model_frequency)
        except (ValueError, TypeError) as e:
            self.save_model_frequency = 100
            GGPrint.print('{}Use save_model_frequency with default value:{} because of error:{}'.format(PREFIX, self.save_model_frequency, e))

        self.summarize_layer_frequency = 1000
        if hparams and 'summarize_layer_frequency' in hparams.keys():
            GGPrint.print('{}Use summarize_layer_frequency in hparams:{}'.format(PREFIX, hparams['summarize_layer_frequency']))
        ...
        try:
            ...
            self.summarize_layer_frequency = 1000
            GGPrint.print('{}Use summarize_layer_frequency with default value:{} because of error:{}'.format(PREFIX, self.summarize_layer_frequency, e))

        self.summarize_layer_name_list = None
        if hparams and 'summarize_layer_name_list' in hparams.keys():
            GGPrint.print('{}Use summarize_layer_name_list in hparams:{}'.format(PREFIX, hparams['summarize_layer_name_list']))
        ...
        if self.summarize_layer_name_list is not None:
            try:
                ...
                self.summarize_layer_name_list = None
                GGPrint.print('{}Use summarize_layer_name_list with default value:{} because of error:{}'.format(PREFIX,
    ...
    def train(self, iter_to=10000, learning_rate=1e-4, batch_size=128, dropout_ratio=0.5, l1_norm_reg_ratio=0.0, save_file_path=None, report_dir_path=None):
        ...
        GGPrint.print('test_labels.shape:{}'.format(test_labels.shape))
        ...
        # GGPrint.print('test_batch_size: {}, self.evaluate_in_minibatch: {}, len(test_data): {}'.format(test_batch_size, self.evaluate_in_minibatch, len(test_data)))
        ...
        # GGPrint.print('len(test_batch_index_list): {}, test_batch_index_list: {}'.format(len(test_batch_index_list), test_batch_index_list))
        ...
        for i in range(iter_to):
            if (not self.test_only_mode) and (not self.prediction_mode):
                ...
                # train
                # GGPrint.print('{}, output_batch.shape:{}'.format(self.global_iter, output_batch.shape))
                self.train_step.run(
            ...
            if is_iter_to_summarize_layer and report_dir_path:
                ...
                for test_batch_index in test_batch_index_list:
                    # GGPrint.print('test_batch_index: {}'.format(test_batch_index))
            ...
            if is_iter_to_test_and_report:
                ...
                if self.model_type == 'CLASSIFICATION':
                    GGPrint.print('test_labels: {}'.format(test_labels))

                    _accuracy = accuracy_score(test_labels, y_label_estimated)
                    GGPrint.print('_accuracy: {}'.format(_accuracy))

                    GGPrint.print('========================================')
                    GGPrint.print('step %d, test accuracy %g' % (self.global_iter, _accuracy))
                    GGPrint.print('========================================')
                ...
                if report_dir_path:
                    ...
                    for _offset in [0]:
                        ...
                        if self.calc_cc_errors and self.op_errors is not None:
                            ...
                            GGPrint.print('error_name:{}, error_to_plot:{}, cc_error_name:{}, cc_error:{}'.format(error_name, error_to_plot, cc_error_name, cc_error))
                ...
                if save_file_path and not (self.test_only_mode or self.prediction_mode):
                    ...
                    if self.export_to_onnx:
                        if tf_major_version < 2:
                            GGPrint.print('ONNX export is not supported with TensorFlow 1.X')
                        else:
        ...
        with tf.name_scope(_name ):
            with tf.name_scope('W_conv'):
                try:
                    ...
                    GGPrint.print('cnn_weight_stddev is set with {}'.format(cnn_weight_stddev))
                except AssertionError as e:
                    cnn_weight_stddev = NNModel.DEFAULT_WEIGHT_STDDEV
                    GGPrint.print('cnn_weight_stddev is set with {} because of error:{}'.format(cnn_weight_stddev, e))
            ...
            if not has_batch_norm:
                with tf.name_scope('b_conv'):
                    try:
                        ...
                        GGPrint.print('cnn_bias_value is set with {}'.format(cnn_bias_value))
                    except AssertionError as e:
                        cnn_bias_value = NNModel.DEFAULT_BIAS_VALUE
                        GGPrint.print('cnn_bias_value is set with {} because of error:{}'.format(cnn_bias_value, e))
                    b_conv = self.bias_variable([conv_out_channels], value=cnn_bias_value, name="b_conv")
                    self.variable_summaries(b_conv)

            GGPrint.print('########## {} ########## input x shape:{} ########## W_conv:{} ########## pool:{} ########## has_batch_norm:{} ########## actf:{} ########## cnn_weight_stddev:{} ########## cnn_bias_value:{} ##########'.format(
    ...
    def define_2d_cnn_model(self, n_layer=5, has_res_net=False, has_non_cnn_net=False):
                ...
                while l < n_cnn_layers:
                    ...
                    with tf.name_scope('model/' + block_name + '/'):
                        ...
                        with tf.name_scope('model/' + block_name + '/' + layer_name + '/'):
                            ...
                            GGPrint.print('pool_res:{}'.format(pool_res))
                            if pool_res > 1:
        ...
        with tf.name_scope('model/' + layer_name + '/'):
            ...
            for l in range(1 + self.num_add_fc_layers):
                ...
                GGPrint.print('########## {} ########## ########## _W_fc:{} ########## _b_fc:{} ########## _fc_layer:{} ########## fc_weight_stddev:{} ########## fc_bias_value:{} ##########'.format(
            ...
            if False:
                ...
                GGPrint.print('########## {} ########## input x shape:{} ########## W_fc2:{} ########## b_fc2:{} ########## output_middle_layer:{} ##########'.format(
    ...
    def pool_residual(self, x, pool):
        ...
        GGPrint.print('pooled_residual shape:{}'.format(pooled_residual.shape))
    ...
    def pad_residual(self, x, pad_channel_size):
        ...
        GGPrint.print('padded_residual shape:{}'.format(padded_residual.shape))
